...
class CLS_Cell(object):
    ...
    def draw(self, scr):
        ...
        if self.cellType == "country":
            ...
            scr.blit(
                self.font.render("$" + str(self.price), True, self.cColor),
                (
                    self.x - self.font.size("$" + str(self.price))[0] // 2,
                    self.y + self.cSize // 3,
                ),
            )
            ...
            scr.blit(
                ...
                (
                    self.x - self.font2.size("START")[0] // 2,
                    self.y - self.font2.size("START")[1] // 2,
                ),
            )
            ...
            scr.blit(
                ...
                (
                    self.x - self.font2.size("Jail")[0] // 2,
                    self.y - self.font2.size("Jail")[1] // 2,
                ),
            )
            ...
            scr.blit(
                ...
                (
                    self.x - self.font.size("Punishment")[0] // 2,
                    self.y + self.cSize // 3,
                ),
            )
            ...
            scr.blit(
                self.font.render(tWords[1].title(), True, (30, 50, 30)),
                (
                    self.x - self.font.size(tWords[1].title())[0] // 2,
                    self.y + self.cSize // 3,
                ),
            )
...
class CLS_Board(object):
    def __init__(self, size, countries, prices, colors):
        ...
        setNum = min(
            random.randint(
                int((iLen * 2 / 3) // setCells), int((iLen * 7 / 8) // setCells)
            ),
            28,
        )
        ...
        for i in range(self.cellNum):
            if self.rev:
                self.cX = (
                    SCREEN_W // 2
                    - (self.cSize + 1)
                    * (self.size + self.cLayer - 1 - 2 * self.cInd)
                    * 3**0.5
                    // 2
                )
                self.cY = (
                    SCREEN_H // 2
                    - 3 * (self.size - self.cLayer - 1) * (self.cSize + 1) // 2
                )
            else:
                self.cX = (
                    SCREEN_W // 2
                ...
                self.cY = (
                    SCREEN_H // 2
                    + 3 * (self.cLayer - self.size + 1) * (self.cSize + 1) // 2
                )
            ...
            if type(info) == type([114, 124]):
                newCell = CLS_Cell(
                    ...
                    self.cSize,
                    "country",
                    self.countries[info[0]][info[1]],
                    self.prices[info[0]][info[1]],
                    self.colors[info[0]],
                )
            else:
