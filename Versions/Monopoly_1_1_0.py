...
class CLS_Cell(object):
    def __init__(self, x, y, cSize, cellType, name, price, cColor):
        ...
        self.font = pygame.font.Font('Data\\Fonts\\GlacialIndifference-Regular.otf', self.cSize * 3 // 11)
        self.font2 = pygame.font.Font('Data\\Fonts\\GlacialIndifference-Regular.otf', self.cSize // 3)

    def draw(self, scr):
        if self.cellType == 'country':
            pygame.draw.polygon(scr, self.cColor, (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (40, 5, 77), (
                (self.x, self.y + self.cSize - 4),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 4, self.y + self.cSize // 2 - 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 4, self.y - self.cSize // 2 + 2),
                (self.x, self.y - self.cSize + 4), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 4, self.y - self.cSize // 2 + 2),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 4, self.y + self.cSize // 2 - 2)))
            ...
            pygame.draw.polygon(scr, (67, 125, 85), (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (103, 150, 111), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font2.render('START', True, (255, 255, 255)),
                     (self.x - self.font2.size('START')[0] // 2, self.y - self.font2.size('START')[1] // 2))
        elif self.cellType == 'jail':
            pygame.draw.polygon(scr, (30, 30, 30), (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (170, 170, 170), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font2.render('Jail', True, (20, 20, 20)),
                     (self.x - self.font2.size('Jail')[0] // 2, self.y - self.font2.size('Jail')[1] // 2))
        elif self.cellType == 'surprise':
            pygame.draw.polygon(scr, (150, 100, 54), (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (210, 190, 170), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font.render('Surprise', True, (110, 70, 24)),
            ...
            pygame.draw.polygon(scr, (0, 0, 0), (
                ...
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (190, 190, 190), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font.render('Punishment', True, (0, 0, 0)),
                     (self.x - self.font.size('Punishment')[0] // 2, self.y + self.cSize // 3))
        elif self.cellType == 'treasure':
            pygame.draw.polygon(scr, (130, 110, 70), (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (230, 219, 200), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font.render('Treasure', True, (50, 50, 20)),
                     (self.x - self.font.size('Treasure')[0] // 2, self.y + self.cSize // 3))
        else:
            tWords = self.cellType.split()
            pygame.draw.polygon(scr, (30, 50, 30), (
                (self.x, self.y + self.cSize), (self.x - self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2), \
                (self.x - self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2), (self.x, self.y - self.cSize), \
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y - self.cSize // 2),
                (self.x + self.cSize * 3 ** 0.5 // 2, self.y + self.cSize // 2)))
            pygame.draw.polygon(scr, (230, 250, 230), (
                (self.x, self.y + self.cSize - 2),
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y + self.cSize // 2 - 1), \
                (self.x - self.cSize * 3 ** 0.5 // 2 + 2, self.y - self.cSize // 2 + 1),
                (self.x, self.y - self.cSize + 2), \
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y - self.cSize // 2 + 1),
                (self.x + self.cSize * 3 ** 0.5 // 2 - 2, self.y + self.cSize // 2 - 1)))
            scr.blit(self.font.render(tWords[0].title(), True, (30, 50, 30)),
                     (self.x - self.font.size(tWords[0].title())[0] // 2, self.y))
            scr.blit(self.font.render(tWords[1].title(), True, (30, 50, 30)),
                     (self.x - self.font.size(tWords[1].title())[0] // 2, self.y + self.cSize // 3))
    ...
    def __init__(self, size, countries, prices, colors):
        ...
        for i in self.indices:
            self.distribution[i] = random.choice(
                ['surprise', 'treasure', 'punishment', 'electric company', 'water company', 'oil company'])
        ...
        for i in range(self.cellNum):
            ...
            if type(info) == type([114, 124]):
                newCell = CLS_Cell(self.cX, self.cY, self.cSize, 'country', self.countries[info[0]][info[1]],
                                   self.prices[info[0]][info[1]], self.colors[info[0]])
            else:
